...
このフィールドは署名をそれぞれ 72 文字の署名にわけ、それぞれを評価する。

これは一つのアクションが複数の承認を一つのトランザクション内で必要とするときに有効である。
...
ナンスは通常の Ethereum トランザクションと同様の役割を持っている。

トランザクションはナンスに 1 足したものと一致した場合にのみ実行され、`lastNonce`は現在のナンスにアップデートされる。

これはトランザクションがおかしな順番で実行されたり 1 度以上実行されることを防止する。

コントラクトはナンスを含まない(ナンスが 0)のトランザクションのみ受け付けることもあります。その場合、2 回以上実行されることがないようにコントラクトはトランザクションの完全なハッシュを取っておく必要がある。

ユーザーはおかしな順番やまったく順序を考慮しないで実行されるトランザクションにも署名できるので、これはコントラクトにより一層柔軟性をもたらしますが、各トランザクションがより一層メモリを消費するようにもなる。

これは例えば、ユーザーがナンスがわからないような未来に実行したいトランザクションや、実行されることが保証されていなかったり問題があったりするときのみに実行されるトランザクションのために使われうる。
...
コントラクトアカウントの標準化は ERC の管轄外であり、今後取りうる実装の設計のために存在する。

自分で`_execBytes`を評価することは必須ではなく、コントラクトアカウントのロジックに依存するため、評価はローカルに実行される可能性もあります。

### ガスの計算と返金について

これらを実装しているコントラクトは gas 利用の記録を取っておく必要がある。こうするための 1 つの方法として、まず関数実行の始めに`gasLeft()`を呼び出して置き、実行後差分を確認する方法が存在する。

その後コントラクトは`_gasRelayer`に利用した gas 代(`gasSpent * gasPrice`)を送信し、そしてアカウントはメッセージをデプロイすることになる。

`_gasRelayer`の値が 0 のときは、資金は必ず`block.coinbase`へと転送される。

資金が足りないまたは利用した gas 量が`gasLimit`を超えてしまった場合は、トランザクションは必ずリバートされる必要がある。

トランザクションが内部で失敗した場合でも nonce はインクリメントされ gas 代を払う必要がある。

コントラクトは ETH やコントラクトに取って望ましくないトークンをサポートすることは義務付けられておらず、彼らに取って望ましい少数のトークンによる返金を受け付けるだけにするように実装することも可能である。
